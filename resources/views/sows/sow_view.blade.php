...
<div id="accordion">
    <div class="card">
        <div class="card-header" id="headingTwo" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="true" aria-controls="collapseOne">
          <h5 class="mb-0">
            <h3>
                Annexure
            </h3>
...
<div id="MasterTextDiv">
@foreach ($masters as $key => $val)
{!! Form::hidden('master_'.$key,$val, array('id'=>'master_'.$key,'class'=>'Mastertext')) !!}
@endforeach
</div>

<!-- Text diffrence Highlighter  -->
<script>
 ...
 window.onload = htmlcheck;
 function htmlcheck() {  
        var NewTextItem = document.getElementsByClassName('NewText'); 
        for (var i = 0; i < NewTextItem.length; ++i) {
            var InputName       = NewTextItem[i].getAttribute('data-new-input');   
            var NewInputHtml    = NewTextItem[i].innerHTML;
            var elementExists   = document.getElementById('master_'+InputName);
            if(elementExists){
                  var MasterInputHtml = document.getElementById('master_'+InputName).value;
                  NewInputHtml = NewInputHtml.replace(/&amp;/g, "&");    
                  output = htmldiff(MasterInputHtml, NewInputHtml);   
                  NewTextItem[i].innerHTML = output;
                  //console.log(output);
            }
        } 
      }      
...
del {
 ...
 <!-- End of text diffrence Highlighter  -->
